...
# For STM32 you could use ExtiInput and configure in Rust.
...
cpi = 1600 # Optional, defaults to 1600
rot_trans_angle = -15 # Optional, defaults to 0
liftoff_dist = 8 # Optional, defaults to 0
proc_invert_x = true
# proc_invert_y = true
# proc_swap_xy = true
```
...
```rust
    ...
    let mut pmw33xx_device = PointingDevice::<Pmw33xx<_, _, _, Pmw3360Spec>>::new(POINTING_DEV_ID, spi_bus, cs, Some(motion), sensor_config);
    // for PMW3389
    let mut pmw33xx_device = PointingDevice::<Pmw33xx<_, _, _, Pmw3389Spec>>::new(POINTING_DEV_ID, spi_bus, cs, Some(motion), sensor_config);
